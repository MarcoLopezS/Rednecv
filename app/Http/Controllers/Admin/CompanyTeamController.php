...
use Rednecv\Repositories\TeamRepo;
...
class CompanyTeamController extends Controller {
    ...
    {
        $posts = $this->teamRepo->findAndPaginate($request);

        return view('admin.company-team.list', compact('posts'));
    }
    ...
    public function store(Request $request)
    {
        ...
        {
            $this->teamRepo->CrearCarpeta();
            $ruta = "upload/".$this->teamRepo->FechaCarpeta();
            $archivo = $request->file('imagen');
            $imagen = $this->teamRepo->FileMove($archivo, $ruta);
            $imagen_carpeta = $this->teamRepo->FechaCarpeta();
        }else{
            $imagen = "";
            $imagen_carpeta = "";
        }        
        ...
        //GUARDAR DATOS
        $post = new Team($request->all());
        $post->imagen = $imagen;
        $post->imagen_carpeta = $imagen_carpeta;
        $this->teamRepo->create($post, $request->all());

        //MENSAJE
        flash()->success('El registro se agregó satisfactoriamente.');

        //REDIRECCIONAR A PAGINA PARA VER DATOS
        return redirect()->route('admin.company.team.index');
    }
    ...
    public function show($id)
    {
        ...
        return view('admin.company-team.show', compact('post'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('admin.company-team.edit', compact('post'));
    }
    ...
    public function update($id, Request $request)
    {
        ...
        $post->imagen = $imagen;
        $post->imagen_carpeta = $imagen_carpeta;
        $this->teamRepo->update($post, $request->except('imagen'));
    ...
    {
        $url = $this->teamRepo->SlugUrl($request->input('ajaxTitulo'));
